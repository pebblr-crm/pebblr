#!/usr/bin/env bash
# scripts/e2e-web-v2.sh — Run Playwright integration tests against the Kind cluster.
#
# Prerequisites:
...
# Switch to v2 UI by hitting /?ui=v2 to set the cookie (validates the endpoint exists).
echo "==> Verifying v2 SPA is served..."
HTTP_STATUS=$(curl -s -o /dev/null -w '%{http_code}' -L "http://127.0.0.1:${LOCAL_PORT}/?ui=v2")
if [[ "$HTTP_STATUS" != "200" ]]; then
  echo "WARNING: /?ui=v2 returned HTTP ${HTTP_STATUS} (expected 200)"
fi
