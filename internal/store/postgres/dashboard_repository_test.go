...
// --- appendDashboardFilter tests ---

func TestAppendDashboardFilter_AllFields(t *testing.T) {
	t.Parallel()
	filter := store.DashboardFilter{
		DateFrom: testTime(),
		DateTo:   testTime().Add(24 * time.Hour),
	...
	conds, args, nextIdx := appendDashboardFilter(nil, nil, 1, filter)
	if len(conds) != 4 {
	...
	if nextIdx != 5 {
		t.Errorf("expected nextIdx 5, got %d", nextIdx)
	}
}

func TestAppendDashboardFilter_EmptyFilter(t *testing.T) {
	t.Parallel()
	filter := store.DashboardFilter{}
...
func TestAppendDashboardFilter_OnlyDates(t *testing.T) {
	t.Parallel()
	filter := store.DashboardFilter{
		DateFrom: testTime(),
		DateTo:   testTime().Add(24 * time.Hour),
...
// --- activityScopeConditions / targetScopeConditions tests ---

func TestActivityScopeConditions_AllActivities(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{AllActivities: true}
	sql, args, idx := activityScopeConditions(scope, nil, 1)
	if sql != "" {
		t.Errorf("expected empty SQL for AllActivities, got %q", sql)
	}
	if len(args) != 0 {
		t.Errorf("expected 0 args, got %d", len(args))
	}
...
func TestActivityScopeConditions_WithCreatorIDs(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{CreatorIDs: []string{"u1", "u2"}}
	sql, args, idx := activityScopeConditions(scope, nil, 1)
	if sql == "" {
...
func TestActivityScopeConditions_WithTeamIDs(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{TeamIDs: []string{"t1"}}
	sql, args, idx := activityScopeConditions(scope, nil, 1)
	if sql == "" {
		t.Error("expected non-empty SQL")
	}
...
func TestActivityScopeConditions_EmptyScope(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{}
	sql, args, idx := activityScopeConditions(scope, nil, 1)
	if sql != "" {
		t.Errorf("expected empty SQL for empty scope, got %q", sql)
	}
	if len(args) != 0 {
		t.Errorf("expected 0 args, got %d", len(args))
	}
...
func TestActivityScopeConditionsAliased_WithAlias(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{CreatorIDs: []string{"u1"}}
	sql, _, _ := activityScopeConditionsAliased("a", scope, nil, 1)
	if sql == "" {
		t.Error("expected non-empty SQL")
	}
	// Should contain the alias prefix "a."
	if len(sql) < 3 {
		t.Error("SQL too short to contain alias")
	}
}

func TestActivityScopeConditionsAliased_NoAlias(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{CreatorIDs: []string{"u1"}}
	sql, _, _ := activityScopeConditionsAliased("", scope, nil, 1)
	if sql == "" {
		t.Error("expected non-empty SQL")
	}
}

func TestActivityScopeConditions_CreatorsAndTeams(t *testing.T) {
	t.Parallel()
	scope := rbac.ActivityScope{CreatorIDs: []string{"u1"}, TeamIDs: []string{"t1"}}
	sql, args, idx := activityScopeConditions(scope, nil, 1)
	if sql == "" {
		t.Error("expected non-empty SQL")
	}
...
func TestTargetScopeConditions_AllTargets(t *testing.T) {
	t.Parallel()
	scope := rbac.TargetScope{AllTargets: true}
	sql, args, idx := targetScopeConditions(scope, nil, 1)
	if sql != "" {
		t.Errorf("expected empty SQL for AllTargets, got %q", sql)
	}
	if len(args) != 0 {
		t.Errorf("expected 0 args, got %d", len(args))
	}
...
func TestTargetScopeConditions_WithAssigneeIDs(t *testing.T) {
	t.Parallel()
	scope := rbac.TargetScope{AssigneeIDs: []string{"u1"}}
	sql, args, idx := targetScopeConditions(scope, nil, 1)
	if sql == "" {
		t.Error("expected non-empty SQL")
	}
...
func TestTargetScopeConditionsAliased_WithAlias(t *testing.T) {
	t.Parallel()
	scope := rbac.TargetScope{AssigneeIDs: []string{"u1"}, TeamIDs: []string{"t1"}}
	sql, args, idx := targetScopeConditionsAliased("t", scope, nil, 1)
	if sql == "" {
		t.Error("expected non-empty SQL")
	}
	if len(args) != 2 {
...
func TestTargetScopeConditions_EmptyScope(t *testing.T) {
	t.Parallel()
