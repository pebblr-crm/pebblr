...
func validateFieldConfigs(cfg *TenantConfig, fields []FieldConfig, scope string) error {
	...
	for _, f := range fields {
		if f.Key == "" {
			return fmt.Errorf("%s: field key must not be empty", scope)
		}
		if !validTypes[f.Type] {
			return fmt.Errorf("%s: field %q has invalid type %q", scope, f.Key, f.Type)
		}
		if f.OptionsRef != "" && !dbBackedRefs[f.OptionsRef] && cfg.ResolveOptions(f.OptionsRef) == nil {
			return fmt.Errorf("%s: field %q references unknown options_ref %q", scope, f.Key, f.OptionsRef)
		}
	}
	return nil
}
